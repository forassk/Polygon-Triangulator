...
void merge (Point* polygon, int l, int m, int h) {
	...
	for (int i = 0; i < n1; i ++) l_polygon[i] = polygon[l + i];
	for (int i = 0; i < n2; i ++) r_polygon[i] = polygon[m + 1 + i];
	int i = 0, j = 0, k = l;
	while (i < n1 && j < n2) {
...
int* form_chain (Point* polygon, int n) {
	...
	for (int i = 1; i <= n-2; i ++) {
		...
		if (j0_left == i0 || j0_right == i0) chain[i] = chain[j];
		else chain[i] = (chain[j] + 1) % 2;
	}
	return chain;
}
