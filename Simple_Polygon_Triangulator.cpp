...
Diagonal* triangulate (Diagonal* diag, Point* polygon, int n, Ear* ear, bool* present) {
	if (nn == 3) return diag; // if number of unvisited vertices is 3, triangulation is done, because polygon has reduced to a triangle
	int i = ear -> idx;
	present[i] = false;
	nn --;
	ear = ear -> next;
